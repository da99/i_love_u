...
(function() {
  ...
  Argument = (function() {
    ...
    function Argument(txt) {
      var regex_and_types;
      this.rw('user_pattern', txt);
      regex_and_types = Argument.user_pattern_to_types(txt);
      if (regex_and_types) {
        this.rw('regex', regex_and_types[0]);
        this.rw('types', regex_and_types[1]);
        this.rw('first_type', this.types()[0]);
      }
      this.is_start(false);
      this.is_end(false);
    }
    ...
    Argument.prototype.extract_args = function(raw_txt, line) {
      var args, captures, compiled_arg, ctxt, env, i, mismatch, origin_args, raw_s, s, target, txt, _i, _len;
      env = line.calling_env();
      if (!raw_txt.value) {
        raw_txt = new englishy.Stringy(raw_txt);
      }
      txt = raw_txt.value();
      ctxt = env.get_if_data(function(mess) {
        mess.name(txt);
      ...
      if (this.regex()) {
        ...
        for (_i = 0, _len = captures.length; _i < _len; _i++) {
          ...
          compiled_arg = s.is_quoted() ? s.value() : env.get_if_data(function(mess) {
            mess.name(s.value());
            return mess.line(line);
          });
    ...
    Argument.Noun = {
      ...
      is_a_match_with: function(unk) {
        return (!!unk.is_a_noun) && unk.is_a_noun();
      },
      convert: function(unk) {
        return unk;
      }
