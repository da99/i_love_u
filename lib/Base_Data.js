// Generated by CoffeeScript 1.3.3
(function() {

  module.exports = {};

  module.exports.i_love_u = function(ilu) {
    ilu.vars().push_name_and_value(/^Block_Text_Line_[0-9]+$/, function(name, env, line) {
      var block, num, val;
      block = line.block();
      if (!block) {
        throw new Error("Block is not defined.");
      }
      num = parseInt(name.split('_').pop());
      return val = block.text_line(num);
    });
    ilu.vars().push_name_and_value(/^Block_List_[0-9]+$/, function(name, env, line) {
      var block, list, num, str, tokens, v, _i, _len;
      block = line.block();
      if (!block) {
        throw new Error("Block is not defined.");
      }
      num = parseInt(name.split('_').pop());
      str = block.text_line(num).strip();
      tokens = _.flatten(new englishy.Englishy(str + '.').to_tokens());
      list = [];
      for (_i = 0, _len = tokens.length; _i < _len; _i++) {
        v = tokens[_i];
        if (v.is_quoted()) {
          list.push(v.value());
        } else {
          list.push(env.get_if_data(v.value(), line));
        }
      }
      return list;
    });
    return ilu.vars().push_name_and_value("List", {
      is_a_noun: function() {
        return true;
      },
      target: function() {
        var _ref;
        return (_ref = this._target_) != null ? _ref : this._target_ = new humane_list();
      },
      insert: function(pos, val) {
        return this.target().push(pos, val);
      },
      values: function() {
        return this.target().values();
      }
    });
  };

}).call(this);
