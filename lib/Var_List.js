  ...
  Var_List = (function() {
    ...
    Var_List.prototype.get = function(name, line) {
      ...
      if (!this.env().envs().is_read_local()) {
        _var = this.env().envs().read().vars().get(name, line);
      }
      if (!_var) {
        _var = this.vars[name] ? this.vars[name] : _.find(this.pattern_based(), function(v) {
          return v.is_named(name);
        });
      }
      if (_var && _var.is_local_only() && _var.env() !== line.calling_env()) {
        _var = void 0;
      }
      return _var;
    };
    ...
    Var_List.prototype.run_line_tokens = function(pair) {
      ...
      if (!this.env().is_read_local()) {
        match = this.env().envs().read().run_line_tokens(pair, env);
      }
