...
(function() {
  var LOOP_LIMIT, Procedure, Var, arr_surgeon, as_num, catch_err, compare, comparison_type_cast, else_false, englishy, equals, funcy_perm, i_love_u, if_false, if_true, md_num, not_equals, run_comparison_on_match, rw, string_da99, while_loop, word_is_now, word_is_word, _, _do_,
    __slice = [].slice;
  ...
  while_loop.write('procedure', function(match) {
    ...
    if (prev && prev.from_do) {
      code = prev.code;
    } else {
      code = match.code() && match.code().text();
    }
    if (!code) {
      return match.is_a_match(false);
    } else {
      return while_loop._while_(match, code);
    }
  });

  while_loop._while_ = function(match, code) {
    var env, luv, re_run, tokens, val;
    env = match.env();
    val = match.args()[0];
    tokens = null;
