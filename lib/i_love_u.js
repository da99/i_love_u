...
(function() {
  ...
  exports.i_love_u = i_love_u = (function() {
    ...
    i_love_u.data = function(name, val) {
      if (arguments.length === 0) {
        return this._data_;
      } else if (arguments.length === 1) {
        return _.find(this._data_, function(v) {
          return v[0] === name;
        });
      } else {
        return this._data_.push(new Var(name, val));
      }
    };

    i_love_u.procs = function(proc) {
      if (arguments.length === 0) {
        return this._procs_;
      }
      switch (proc.position()) {
        case 'top':
          return this._procs_.unshift(proc);
        case 'middle':
          return this._procs_.splice(Math.ceil(this._procs_.length / 2), 0, proc);
        case 'bottom':
          return this._procs_.push(proc);
        default:
    ...
    i_love_u.read_able("outside_scope", 'pattern', 'data', 'procs', 'data', 'scope', 'loop_total');
    ...
    function i_love_u(str, env) {
      ...
      if (env) {
        ...
        this.rw('procs', [].concat(this.constructor.procs()));
        this.rw('outside_scope', "none");
        _ref = this.constructor.data();
