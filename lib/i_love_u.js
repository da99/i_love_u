...
(function() {
  var LOOP_LIMIT, Line, Procedure, Var, arr_surgeon, as_num, boolean_type_cast, catch_err, compare, comparison_type_cast, else_false, englishy, equals, funcy_perm, i_love_u, if_true, is_boolean_string, md_num, not_equals, run_comparison_on_match, rw, string_da99, while_loop, word_is_now, word_is_word, _, _do_,
    __slice = [].slice;
  ...
  while_loop.write('procedure', function(match) {
    var ans, code, env, prev;
    env = match.env();
    prev = _.last(env.scope());
    ans = match.args()[0];
    code = prev && prev.from_do ? prev.code : match.line().code();
    if (!code) {
      return match.is_a_match(false);
    }
    if (ans) {
